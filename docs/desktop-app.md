...
桌面端从 `0.1.1` 开始接入 Tauri 官方 updater。默认采用确认式更新：有新版本时先在后台下载更新包，下载完成后在顶部状态区显示“新版本已下载”，由用户选择“安装并重启”或“稍后”；没有新版本时不显示任何更新入口。
...
例如发布 `desktop-v0.1.5` 前，上面三个地方都应改成：
...
- 桌面端启动后会检查 GitHub Releases 更新，发现新版本后先后台下载更新包；下载完成后由用户选择是否安装并重启；已经是最新版本时不显示更新提示。
